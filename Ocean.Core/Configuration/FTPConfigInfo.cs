...
namespace Ocean.Core.Configuration
{
    ...
    public class FTPConfigInfo : IConfigInfo
    {
        ...
        private int m_ident = 1;//唯一标识

        private string m_name = ""; //名称,如forumattach:论坛附件,spaceattach:空间附件,album:相册等

        private string m_serveraddress = ""; //服务器地址=localhost：则为本机其他硬盘
        ...
        private string m_remoteurl2 = ""; //远程访问 URL2

        private int m_reservelocalattach = 0; //是否保留本地附件.  0:不保留  1:保留

        private int m_reserveremoteattach = 1; //删除时是否保留远程附件    0为不保留  1为保留

        private string m_assemblytype = "";//FTP插件类名:为空时，则调用系统默认

        private int m_enable = 0;//是否启用

        private int m_default = 0;//是否默认

        private int m_pathformat = 0;//存储路径格式
        #endregion
        ...
        {
            get { return m_remoteurl2; }
            set { m_remoteurl2 = value; }
        }
        ...
        {
            get { return m_pathformat; }
            set { m_pathformat = value; }
        }
        #endregion
