    ...
    public class ConfigurationProvider<TSettings> : IConfigurationProvider<TSettings> where TSettings : ISettings, new()
    {
        ...
        private void BuildConfiguration()
        {
            ...
            var properties = from prop in typeof(TSettings).GetProperties()
                             ...
                             where setting != null
                             where CommonHelper.GetOceanCustomTypeConverter(prop.PropertyType).CanConvertFrom(typeof(string))
                             where CommonHelper.GetOceanCustomTypeConverter(prop.PropertyType).IsValid(setting)
                             let value = CommonHelper.GetOceanCustomTypeConverter(prop.PropertyType).ConvertFromInvariantString(setting)
