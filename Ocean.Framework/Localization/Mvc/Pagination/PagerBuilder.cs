...
namespace Ocean.Framework.Mvc.Pagination
{
    internal class PagerBuilder
    {
        ...
        private readonly string _siteLang;
        ...
        private const string JqCheckScript = "if(jQuery(document)==null){alert(\"jQuery library not detected, please make sure it loaded prior to MvcPager!\");}";
        private const string GoToPageScript = "\nfunction _MvcPager_GoToPage(_pib,_mp){ var pageIndex = $(_pib).val(); if(!$(_pib).is(\"select\")){ var r = new RegExp(\"^[0-9]+$\"); if (!r.test(pageIndex)) { alert(\"%InvalidPageIndexErrorMessage%\");return;}else if(parseInt(pageIndex)<1||parseInt(pageIndex)>_mp){alert(\"%PageIndexOutOfRangeErrorMessage%\");return;}}var _hl=$(\"#\" + $(_pib).attr(\"id\") + \"link a\").first();var _lh=_hl.attr(\"href\");$(_hl).attr(\"href\",_lh.replace('" + ScriptPageIndexName + "',pageIndex));$(_hl).click();$(_hl).attr('href',_lh);}";
        //private const string GoToPageScript = "\nfunction _MvcPager_GoToPage(_pib,_mp){ var pageIndex = $(_pib).val(); if(!$(_pib).is(\"select\")){ var r = new RegExp(\"^\\s*(\\d+)\\s*$\"); if (!r.test(pageIndex)) { alert(\"%InvalidPageIndexErrorMessage%\");return;}else if(RegExp.$1<1||RegExp.$1>_mp){alert(\"%PageIndexOutOfRangeErrorMessage%\");return;}}var _hl=$(\"#\" + $(_pib).attr(\"id\") + \"link\").find(\"a\")[0];var _lh=$(_hl).attr('href');$(_hl).attr(\"href\",_lh.replace('" + ScriptPageIndexName + "',pageIndex));$(_hl).attr('href',_lh);$(_hl).click();}";
        private const string KeyDownScript = "\nfunction _MvcPager_Keydown(e){var _kc,_pib;if(window.event){_kc=e.keyCode;_pib=e.srcElement;}else if(e.which){_kc=e.which;_pib=e.target;}var validKey=(_kc==8||_kc==46||_kc==37||_kc==39||(_kc>=48&&_kc<=57)||(_kc>=96&&_kc<=105));if(!validKey){if(_kc==13){ _MvcPager_GoToPage(_pib,%TotalPageCount%);}if(e.preventDefault){e.preventDefault();}else{event.returnValue=false;\n}}}";
        ...
        private void AddPageNumbers(ICollection<PagerItem> results)
        {
            for (var pageIndex = _startPageIndex; pageIndex <= _endPageIndex; pageIndex++)
            {
                ...
                if (!string.IsNullOrEmpty(_pagerOptions.PageNumberFormatString) && _pagerOptions.PageNumberFormatString.Contains("{0}"))
                    text = string.Format(_pagerOptions.PageNumberFormatString, text);
                if (pageIndex == _pageIndex && !string.IsNullOrEmpty(_pagerOptions.CurrentPageNumberCss))
                    text = String.Format("<span class=\"{0}\">{1}</span>", _pagerOptions.CurrentPageNumberCss, text);
                else if (!string.IsNullOrEmpty(_pagerOptions.PageNumberCss))
                    text = String.Format("<span class=\"{0}\">{1}</span>", _pagerOptions.PageNumberCss, text);
        ...
        #region 显示统计信息-zhenhua.du 2011.12.15
        private StringBuilder AddTotalInfo()
        {
            string tagSpan = string.Format("<span{0}>{1}</span>\n"
                , !string.IsNullOrEmpty(_pagerOptions.TotalInfoWrapperCss)?" class=\"" + _pagerOptions.TotalInfoWrapperCss + "\"":""
                , _pagerOptions.TotalInfoFormatString);
            StringBuilder isb = new StringBuilder(tagSpan);
            isb = isb.Replace("%_PageIndex_%", _pageIndex.ToString());
            isb = isb.Replace("%_TotalPage_%", _totalPageCount.ToString());
        ...
        private string GenerateUrl(int pageIndex)
        {
            ...
            //Add sitepage
            routeValues["sitepage"] = _pageName??"";
            // Return link
            ...
            #region 多语言链接处理-vebin.h-2011.11.14
            if (!string.IsNullOrEmpty(_siteLang))
            {
                return "/" + _siteLang + urlHelper.RouteUrl(routeValues);
            }
            #endregion
        ...
        internal MvcHtmlString RenderPager()
        {
            ...
            string _url = _html.ViewContext.HttpContext.Request.RawUrl;
            int designOrPublish = 0;
            //需改
            //if (Utils.ObjectToInt(HttpContext.Current.Items["designruntime"])>0)
            //{
            //    designOrPublish = 1;
            //}
            ...
            if (_msAjaxPaging)
            {
                ...
                {
                    sb.Append(GenerateMsAjaxPagerElement(item,designOrPublish));
                }
            }
            else if (_pagerOptions.UseJqueryAjax)
            {
                foreach (PagerItem item in pagerItems)
                {
                    sb.Append(GenerateJqAjaxPagerElement(item,designOrPublish));
                }
            }
            else
            {
                foreach (PagerItem item in pagerItems)
                {
                    sb.Append(GeneratePagerElement(item, designOrPublish));
                }
            }
            /**显示统计信息-zhenhua.du 2011.12.15**/
            if (!_pagerOptions.ShowPageIndexBox)
            {
                sb.Length -= _pagerOptions.SeparatorHtml.Length;
            }
            if (_pagerOptions.ShowInfo)
            {
                sb.Append(AddTotalInfo().ToString());
            }
            ...
            return MvcHtmlString.Create(CopyrightText + tb.ToString(TagRenderMode.Normal) + CopyrightText);//vebin.h-2012.9.27去掉分页脚本生成: + pagerScript
        }
        ...
        private string BuildGoToPageSection(ref string pagerScript)
        {
            ...
            //    designOrPublish = 1;
            //}
            string scriptLink = GenerateAnchor(new PagerItem("0", 0, false, PagerItemType.NumericPage), designOrPublish);
            ...
            /**增加GoToPageSectionWrapperCss控制ＧＯ样式-zhenhua.du 2012.2.9**/
            if (_pagerOptions.ShowGoButton)
                piBuilder.AppendFormat(
                    "<input type=\"button\"{2} value=\"{0}\" onclick=\"_MvcPager_GoToPage('#{1}'," + _totalPageCount + ")\"/>",
                    _pagerOptions.GoButtonText
                    , controlId + "_pib", !string.IsNullOrEmpty(_pagerOptions.GoToPageSectionWrapperCss)?" class=\"" + _pagerOptions.GoToPageSectionWrapperCss + "\"":"");
            ...
            if (!string.IsNullOrEmpty(_pagerOptions.GoToPageSectionWrapperFormatString))
            {
                outHtml = string.Format(
                    _pagerOptions.GoToPageSectionWrapperFormatString,
                    piBuilder);
            }
            else
        ...
        private MvcHtmlString GeneratePagerElement(PagerItem item,int designOrPublish)
        {
            //pager item link
            string url = _pagerOptions.UseCurrentPageUrl ? GetCurrentPageUrl(item.PageIndex,designOrPublish) : GenerateUrl(item.PageIndex);
        ...
        private MvcHtmlString GenerateMsAjaxPagerElement(PagerItem item,int designOrPublish)
        {
        ...
        #region 使用当前页面-zhenhua.du 2011.12.15
        /// <summary>
        /// 生成分页链接
        /// </summary>
        /// <param name="pageIndex">当前页</param>
        /// <param name="deisnOrPublish">设计时或者发布时[1，2]</param>
        /// <returns></returns>
        private string GetCurrentPageUrl(int pageIndex,int desinOrPublish)
        {
            //return null if  page index larger than total page count or page index is current page index
            if (pageIndex > _totalPageCount || pageIndex == _pageIndex)
                return null;
            ...
            foreach(string key in rq.Keys)
            {
                if (key !=null && key != _pagerOptions.PageIndexParameterName && !routeValues.ContainsKey(key))
                {
                    routeValues[key] = rq[key];
                }
            }
            routeValues[_pagerOptions.PageIndexParameterName] = pi;
            StringBuilder urlParam = new StringBuilder();
            int i = 0;
            foreach (string key in routeValues.Keys)
            {
                if(i<1)
                    urlParam.AppendFormat("{0}={1}", key, routeValues[key]);
                else
                    urlParam.AppendFormat("&{0}={1}", key, routeValues[key]);
                i++;
            }
            ...
            // Return link
            string url = _html.ViewContext.HttpContext.Request.RawUrl;
            if (desinOrPublish == 1)
            {
                url = "/MAdmin/Design/DesignLayout.htm";
            }
            else if (desinOrPublish == 2)
            {
                url = "";
            }
            else
            ...
            return string.Format("{0}?{1}", url, urlParam.ToString());
        }
        #endregion
    }
}